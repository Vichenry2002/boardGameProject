...
/**
 * JavaFX controller class for the card popup when a card is clicked.
 */
public class CardPopupController implements Initializable {

  private static CardPopupController instance;
  int costs = 0;
  int counter = 1;
  int max1;
  int max2;
  ...
  private int doubleGoldValueActive = 1;
  ...
  public static CardPopupController getInstance() {
    return CardPopupController.instance;
  }
  ...
  protected void onBuyButtonClick() {
    ...
    try {
      blackGold = Integer.parseInt(colorToLabelMap.get("black").getText()) * doubleGoldValueActive;
    } catch (Exception e) {
      blackGold = 0;
    }
    try {
      redGold = Integer.parseInt(colorToLabelMap.get("red").getText()) * doubleGoldValueActive;
    } catch (Exception e) {
      redGold = 0;
    }
    try {
      greenGold = Integer.parseInt(colorToLabelMap.get("green").getText()) * doubleGoldValueActive;
    } catch (Exception e) {
      greenGold = 0;
    }
    try {
      blueGold = Integer.parseInt(colorToLabelMap.get("blue").getText()) * doubleGoldValueActive;
    } catch (Exception e) {
      blueGold = 0;
    }
    try {
      whiteGold = Integer.parseInt(colorToLabelMap.get("white").getText()) * doubleGoldValueActive;
    } catch (Exception e) {
      whiteGold = 0;
    }
    ...
    if (missing == 0) {
      ...
      int goldUsedAmount = blackGold + redGold + greenGold + blueGold + whiteGold
          - freeUsed * doubleGoldValueActive;
      PlayerSelfController.getInstance().usedTokens.add(goldUsedAmount / doubleGoldValueActive);
      ...
      if (currentCard.getType() == OrientCardType.TAKE_L1_CARD) {
        ...
        if (Game.getInstance().player1.hasTradingPost(TradingPostType.FREE_TOKEN_WITH_PURCHASE)) {
  ...
  protected void onReserveButtonClick() {
    ...
    GameBoardController.getInstance().reserveSelection(currentImageId);
    ...
    GameBoardController.getInstance().setSelectedAction(ActionType.RESERVE_CARD);
    PlayerSelfController.getInstance().confirmActionSetActive();
  ...
  public void initialize(URL url, ResourceBundle resourceBundle) {
    CardPopupController.instance = this;
   ...
   *
   * @param imageView TODO add description
   */
  public void setVisible(ImageView imageView) {
    ...
    if (Game.getInstance().player1.hasTradingPost(TradingPostType.DOUBLE_GOLD)) {
      ...
      doubleGoldValueActive = 2;
    } else {
      doubleGoldValue.setVisible(false);
      doubleGoldValueActive = 1;
    }

    currentImageId = imageView.getId();
    currentCard = GameBoardController.getInstance().getCardSlot(imageView).getCard();
    ...
    GameController.getInstance().setCardVisible(true);
  ...
  /**
   * Checks if the card pop up is visible.
   *
   * @return true if the card pop up is visible, false otherwise
   */
  ...
  /**
   * Hides the card popup and makes it inactive.
   */
  public void hide() {
    ...
    GameController.getInstance().setCardVisible(false);
